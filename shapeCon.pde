public class shapeCon{
  ...
  public shapeCon(shape[][] pages,float[] timeData){
      ...
      for(int siI = 0; siI < 2; siI++){
        for(int verI = 0; verI<curPage[shI].sides[siI].length; verI++){
          ...
          for(int xyz = 0; xyz<3;xyz++){
            ...
            curPage[shI].sides[siI][verI][xyz] += deltaData[timeIndex][shI][siI][verI][xyz]/timeData[timeIndex]/frameRate*4;
            // println(" hi");
          }
        }
      }
...
public shapeCon advancedImporter(String fileName, float[] timeDataP){
    try{
